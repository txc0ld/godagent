 ...
 * ```
 */
export class RealGlobalAttention implements IAttentionMechanism {
  ...
  private readonly numGlobalTokens: number;
  ...
  constructor(config?: {
    ...
    if (this.numGlobalTokens < 1) {
      throw new Error(
        `numGlobalTokens must be >= 1, got ${this.numGlobalTokens}`
      );
    }
  ...
  forward(
    ...
    // Validate numGlobalTokens vs seqLen
    if (this.numGlobalTokens >= actualSeqLen) {
      throw new Error(
        `numGlobalTokens (${this.numGlobalTokens}) must be < seqLen (${actualSeqLen})`
      );
    }
  ...
  private computeHeadAttention(
    ...
    // 2. Apply global pattern mask
    // Global positions (i < G) attend to all
    // Regular positions (i >= G) attend only to global positions (j < G)
    for (let i = this.numGlobalTokens; i < seqLen; i++) {
      for (let j = this.numGlobalTokens; j < seqLen; j++) {
        const idx = i * seqLen + j;
        scores[idx] = -Infinity;  // Mask out non-global positions
      }
    }

    // 3. Apply user-provided mask (PyTorch convention: true=attend, false=mask out)
    if (mask) {
      for (let i = 0; i < seqLen; i++) {
        for (let j = 0; j < seqLen; j++) {
          const idx = i * seqLen + j;
