 ...
 * ```
 */
export class RealCausalAttention implements IAttentionMechanism {
  ...
  forward(
    ...
    // Step 2: Multi-head causal attention
    const headOutputs: Float32Array[] = [];

    for (let h = 0; h < this.numHeads; h++) {
      const headOutput = this.computeHeadCausalAttention(
        qProjected,
        kProjected,
        vProjected,
        h,
    ...
    if (hasNaNOrInf(output)) {
      throw new Error('ANTI-009: Causal attention forward pass produced NaN or Inf values');
    }
  ...
  private computeHeadCausalAttention(
    ...
    for (let i = 0; i < seqLen; i++) {
      ...
      // Compute attention scores for positions 0..i only (causal constraint)
      // Positions j > i are masked with -∞
      ...
      for (let j = 0; j < seqLen; j++) {
        ...
        // Additional mask check (e.g., padding mask)
        if (mask && !mask[j]) {
          scores[j] = -Infinity;
          continue;
        }
      ...
      // Stable softmax over valid positions
      let sumExp = 0;
      ...
      for (let d = 0; d < this.headDim; d++) {
        let weightedSum = 0;
        for (let j = 0; j <= i; j++) {  // Only iterate over valid positions
