#!/bin/bash
# Claude Code Pre-Task Hook Wrapper
#
# Implements: TECH-HKS-001 Pre-Task Shell Wrapper
...
log() {
    if [[ "${HOOKS_VERBOSE:-false}" == "true" ]]; then
        echo "[pre-task.sh] $1" >&2
    fi
}

log "Starting pre-task hook (timeout: ${TIMEOUT_S}s)"

# Check if TypeScript hook exists
TS_HOOK="$PROJECT_ROOT/scripts/hooks/pre-task.ts"
if [[ ! -f "$TS_HOOK" ]]; then
    echo "[pre-task.sh] ERROR: TypeScript hook not found: $TS_HOOK" >&2
    exit 2
fi
...
if command -v timeout &> /dev/null; then
    ...
    echo "[pre-task.sh] WARN: timeout command not found, running without timeout" >&2
    npx tsx "$TS_HOOK" "$@"
    EXIT_CODE=$?
fi
...
if [[ $EXIT_CODE -eq 124 ]]; then
    echo "[pre-task.sh] ERROR: Hook timed out after ${TIMEOUT_S}s" >&2
    exit 3
fi

log "Pre-task hook completed (exit: $EXIT_CODE)"
